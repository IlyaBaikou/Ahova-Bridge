...
builder.Services.AddHttpClient<LocalAiDiscoveryService>(client =>
{
...
app.MapPost("/api/v1/setup/verify", async (HttpContext httpContext,
    ...
    CancellationToken cancellationToken) =>
{
    if (!setupSession.IsValid(httpContext.Request.Headers["X-Ahova-Setup-Nonce"]))
        return Results.StatusCode(StatusCodes.Status403Forbidden);
    var status = await state.SnapshotAsync(cancellationToken);
    var checks = new List<BridgeSetupCheck>
    {
        status.ControlPlaneConfigured
            ? new("control-plane", "healthy", "Ahova service address is configured.")
            : new("control-plane", "required", "Ahova service address is missing.",
                "Use the address supplied by Ahova."),
        status.Paired
            ? new("pairing", "healthy", "This Bridge is paired with a family.")
            : new("pairing", "required", "This Bridge is not paired yet.",
                "Create a one-time code in Ahova and enter it in the next step."),
    };
...
app.MapPost("/api/v1/setup/ai/discover", async (DiscoverLocalAiRequest request,
    HttpContext httpContext, SetupSession setupSession,
    LocalAiDiscoveryService discovery, CancellationToken cancellationToken) =>
{
    if (!setupSession.IsValid(httpContext.Request.Headers["X-Ahova-Setup-Nonce"]))
        return Results.StatusCode(StatusCodes.Status403Forbidden);
    return Results.Ok(await discovery.DiscoverAsync(request, cancellationToken));
});
app.MapPut("/api/v1/setup/configuration", async (ConfigureBridgeRequest request,
    ...
    CancellationToken cancellationToken) =>
{
    if (!session.IsValid(httpContext.Request.Headers["X-Ahova-Setup-Nonce"]))
        return Results.StatusCode(StatusCodes.Status403Forbidden);
    try
    {
    ...
    CancellationToken cancellationToken) =>
{
        ...
        or InvalidOperationException or InvalidDataException)
    {
        return Results.Problem(statusCode: StatusCodes.Status503ServiceUnavailable,
            title: "Ahova pairing is unavailable",
    ...
    if (credential is not null)
    {
            ...
            or InvalidOperationException or InvalidDataException)
        {
            if (!httpContext.Request.Query.TryGetValue("forceLocal", out var force)
                ...
                return Results.Problem(statusCode: StatusCodes.Status503ServiceUnavailable,
                    title: "Ahova could not confirm revocation",
                    extensions: new Dictionary<string, object?>
                    {
