...
public sealed class BridgeBoundaryTests : IAsyncLifetime
{
    private readonly string stateDirectory = Path.Combine(Path.GetTempPath(),
        $"ahova-bridge-tests-{Guid.NewGuid():N}");
    ...
    [Fact]
    public void RejectsEnabledAiWithoutAnAllowedModel()
    {
        var errors = BridgeOptionsRules.Validate(new BridgeOptions
        {
        ...
        Assert.Contains(errors, item => item.Contains("at least one", StringComparison.Ordinal));
    }
    ...
    public async Task ChangedStorageMountFailsClosed()
    {
        ...
        await using var factory = Factory(storageRoot: storageRoot);
        _ = factory.CreateClient();
        var storage = factory.Services.GetRequiredService<Ahova.Bridge.Capabilities.Storage.ILocalStorageClient>();
        Assert.True(await storage.IsHealthyAsync(CancellationToken.None));
        File.Delete(Path.Combine(storageRoot, ".ahova-bridge-mount-id"));
    ...
    [Fact]
    public async Task AiDiscoveryIsNonceProtected()
    {
        await using var factory = Factory();
        using var client = factory.CreateClient();
        var session = await client.GetFromJsonAsync<JsonElement>("/api/v1/setup/session");
        using var denied = await client.PostAsJsonAsync("/api/v1/setup/ai/discover", new
        {
            provider = "ollama",
        });
        using var request = new HttpRequestMessage(HttpMethod.Post,
            "/api/v1/setup/ai/discover");
        request.Headers.Add("X-Ahova-Setup-Nonce", session.GetProperty("nonce").GetString());
        request.Content = JsonContent.Create(new { provider = "ollama" });
        using var allowed = await client.SendAsync(request);
